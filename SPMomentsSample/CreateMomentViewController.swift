...
//  SPMomentsSample
//
//  Created by Barry Ma on 2016-05-17.
...
class CreateMomentViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, CTAssetsPickerControllerDelegate {
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row == assetsPickerController.selectedAssets.count {
            ...
            let cell = collectionView.dequeueReusableCellWithReuseIdentifier(AttachMomentPhotoCellReusableIdentifier, forIndexPath: indexPath)
            
            var attachImageView = cell.contentView.viewWithTag(AttachMomentPhotoCellImageViewTag) as? UIImageView
            ...
            if attachImageView == nil {
                ...
                attachImageView?.tag = AttachMomentPhotoCellImageViewTag
    ...
    func assetsPickerController(picker: CTAssetsPickerController!, didFinishPickingAssets assets: [AnyObject]!) {
        ...
        stateLabel?.text = CreateMomentViewController.description(x_photosSelected:selectedAssetsCount, y_photosRemain:MomentUploadPhotoMaxNumber - selectedAssetsCount)
    ...
    private func setupCreateMomentVC() {
        
        self.automaticallyAdjustsScrollViewInsets = false
        
        // 设置nav bar button item
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "取消", style: UIBarButtonItemStyle.Plain, target: self, action: "cancelCreateMoment")
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: "发表", style: UIBarButtonItemStyle.Plain, target: self, action: "requestCreateMoment")
        ...
        collectionView?.registerClass(UICollectionViewCell.self, forCellWithReuseIdentifier:AttachMomentPhotoCellReusableIdentifier)
        ...
        stateLabel?.text = CreateMomentViewController.description(x_photosSelected:0, y_photosRemain:MomentUploadPhotoMaxNumber)
    ...
    @objc private func requestCreateMoment() {
        ...
        let selectedAssets = self.assetsPickerController.selectedAssets
        if selectedAssets?.count > 0 {
            for asset in selectedAssets {
                let image = UIImage(CGImage: asset.defaultRepresentation().fullScreenImage().takeUnretainedValue(), scale: 1, orientation: UIImageOrientation.Up)
                let uploadImageSize = suitableSizeForMomentUploadImageSize(image.size)
                let uploadImage = Toucan(image: image).resize(uploadImageSize, fitMode: Toucan.Resize.FitMode.Clip).image
            }
        }
        
        // 上传动态
        let newMoment = Moment(data: 0)
        newMoment.content = self.textView?.text
        newMoment.author = User(data: 0)
        
        SVProgressHUD.showSuccessWithStatus("发表成功", maskType: SVProgressHUDMaskType.Black)
        self.cancelCreateMoment()
...
//                                }
//                            } else {
//                                print("Upload moment photo error:\(error?.localizedFailureReason)")
//                            }
//                        }
//                    }
//                    
...
//                    let newMoment = Moment(data: 0)
//                    
//                    newMoment.content = strongSelf.textView?.text
//                    newMoment.pictures = photoUrls
//                    newMoment.author = User(data: 0)
//                    
////                    var error: NSError?
////                    do {
...
//        }
    ...
    @objc private func cancelCreateMoment() {
        textView?.resignFirstResponder()
...
let MomentUploadPhotoMaxNumber = 9

let AddPhotoButtonCellButtonTag = 100
let AddPhotoButtonCellReusableIdentifier = "AddPhotoButtonCell"
let AttachMomentPhotoCellImageViewTag = 100
let AttachMomentPhotoCellReusableIdentifier = "AttachMomentPhotoCell"
let uploadTwitterPhotoMaxWidth:CGFloat = 640
let uploadTwitterPhotoMaxheight:CGFloat = 800

func suitableSizeForMomentUploadImageSize(uploadImageSize: CGSize!) -> CGSize! {
